...
parWaitBar = waitbar(0,'ETA: Calculating...',...
    'Name','Sorting dicoms');
...
for i = 8:length(subList)
    ...
    %         desInput,desOutput);
    runDesigner = sprintf('source activate py36 && python designer.py -denoise -extent 5,5,5 -degibbs -rician -mask -prealign -smooth 1.25 -eddy -rpe_header -DKIparams -DTIparams %s %s',...
        desInput,desOutput);
...
%% PARFOR Progress Calculation
    function parProgress(~)
        ...
        %   Calculate ETA
        parEndTime = toc;
        parEndTimes(n_completed+1) = parEndTime;
        meanEndTime = mean(parEndTimes);
        parETA = round((meanEndTime * (nFiles - n_completed))/60,1);
