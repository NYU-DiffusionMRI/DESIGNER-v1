function tensorfitting(root,outdir,detectoutliers,cumulants,dti,dki,wmti,fitconstraints,akc,DKIroot,fitWDKI)
...
%     end
    ...
    if akc
        csfmask = logical(niftiread(fullfile(root,'CSFmask.nii')));
         disp('...running AKC correction')
        [dt,akc_out] = shCorrectDt(dt,dwi,mask,bval,bvec,10,csfmask);
...
%         end
    ...
    function[dt,akc_out] = shCorrectDt(dt,dwi,mask,bval,bvec,lmax,csfmask)
        akc_out = outlierdetection(dt,mask);
        akc_out(isnan(akc_out)) = 0;
        [closeInds, ginds,nani] = naninds(akc_out,lmax,csfmask);
...
%         [~,dt_] = dki_fit(dwitmp,[bvectmp,bvaltmp],logical(akc_out),[0,0,0],[],3);
    ...
    function u = fillnans(u, nanlocations,ginds,nanLinearIndexes)
        ...
        for index = 1 : size(nanlocations,1)
            ...
            for ind = 1:size(nanlocations,2)
                goodValue = goodValue + u(xGood(nanlocations(index,ind)), yGood(nanlocations(index,ind)), zGood(nanlocations(index,ind)));
            end
    ...
    function [indexOfClosest,ginds,nanLinearIndexes] = naninds(nanlocations,nvals,csfmask)
        ...
        nonNanLinearIndexes = setdiff(1:numel(nanlocations), nanLinearIndexes);
        ...
        parfor index = 1 : length(nanLinearIndexes)
            ...
            indexOfClosest(index,:) = sortedIndexes(1:nvals);
        end
    end
